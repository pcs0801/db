...
-- EMP02 트리거 설정 (INSERT 발생 시, '입력됨' 메세지 출력)
CREATE OR REPLACE TRIGGER EMP02_TRG01
AFTER INSERT ON EMP02
BEGIN
    DBMS_OUTPUT.PUT_LINE('입력됨');
END;
/
...
CREATE TABLE SAL01(
    ...
    CONSTRAINT SAL01_EMP01_FK FOREIGN KEY(EMPNO) REFERENCES EMP01(EMPLOYEE_ID)
);

-- SAL01.SALNO는 SEQUENCE NUMBER를 이용하여 자동으로 증가되도록 설정
...
-- EMP01에 신입사원이 등록되면, SAL01 신입사원의 사원번호, 기본월급 2,000,000을 입력(TRIGGER 이용)
CREATE OR REPLACE TRIGGER EMP01_TRG02
AFTER INSERT ON EMP01
FOR EACH ROW
BEGIN
    INSERT INTO SAL01 VALUES(SAL01_SEQ.NEXTVAL, 2000000, :NEW.EMPLOYEE_ID);
    DBMS_OUTPUT.PUT_LINE('자동 입력 완료');
END;
/

INSERT INTO SAL01 VALUES(300, 'DONG', 'KIM', 'KIM', '605.507.9812', '25/06/02', 'IT', 2000, NULL, 124, 50);
SELECT * FROM SAL01;